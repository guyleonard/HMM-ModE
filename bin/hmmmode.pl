...
use lib "$FindBin::Bin/bin"; # Dynamically add bin directory to @INCuse HMMmodE;
...
if ( -e $filename ) {
    ...
    print
        "ERROR::There is no all.fasta file, CREATE it and RE-RUN the code\n";
    exit;
}
...
while ( $seqfilename = <LIST> ) {
    ...
    foreach $clus_no ( sort keys %hash_info ) {
        ...
        if ( scalar @own_seq >= 3 ) {
            ...
            my @other_seqs    = ();
            my @own_ids       = ();
            $clusterObj->loadPath( $clus_no, "./$list.test_samples",
                "./$list.train_samples" );
            $path_train           = $clusterObj->{'path_train'};
            @own_ids              = map { $_->display_id() } @own_seq;
            @hash_index{@own_ids} = ();
            my @params = (
                OUT      => "$path_train/$outfile.aln",
                MAXITERS => 2,
                "QUIET"  => "QUIET"
            );
            my $train_aln = $clusterObj->mAlign( "muscle", \@params, $clus_no,
                $ref_own );

            #### GENERATE TP HMM PROFILE
            print "Generating TP HMM profile for $outfile\n";
            system(
                "hmmbuild --informat afa $path_train/$outfile.hmm $path_train/$outfile.aln"
            );

            ### SEARCH AND SCORE ALL THE TRAINING SEQUENCES AGAINST ALL SEQUENCES
            my $ref_fp_train_hash = $clusterObj->hmmsearchParse( "all.fasta",
                "$path_train/$outfile.hmm", "$outfile.neg" );
            ...
            print CUTOFF "NC", "\t", $max_fp_NC, "\n";
            print CUTOFF "TC", "\t", $min_tp_TC, "\n";
            ...
            if ( defined $fp_ids[2] ) {
                ...
                if ( scalar @fp_ids > 200 ) {
                    system "makeblastdb -in $file -dbtype prot -parse_seqids";
                    system "blastp -query $file -db $file -out $file.blast";

              # system "formatdb -p T -i $file -o T";
              # system "blastall  -i $file -p blastp -d $file -o $file.blast";
                    system
                        "mcxdeblast --score=b --sort=a --bcut=5 $file.blast";
                    system "mcxassemble -b $file.blast -q -r max --map";
                    system
                        "mcl $file.blast.sym -scheme 7 -I 1.2 -o $file.clus --append-log=yes";
                    system
                        "rm $file.blast $path_train/*.p* $path_train/*.err $path_train/*.map $path_train/*.raw $path_train/*.hdr";
                    $clusfile = "$file.clus";
                    $tabfile  = "$file.blast.tab";
                    my $fpclusterObj = HMMmodE->new(
                        -file     => $file,
                        -clusType => 'MCL',
                        -clusFile => $clusfile,
                        -tabFile  => $tabfile
                    ...
                    foreach my $clusno (@clus) {
                        my @temp = ();
                        my $percent_cluster
                            = $hash_info{$clusno} / $tot_fp_seq;
                        my $seq_no = int( $percent_cluster * 200 );
                        my $ref_fp_clus_seq
                            = $fpclusterObj->{'cluster_seq'}{$clusno};
                        my @fp_clus_seq = @$ref_fp_clus_seq;
                        my @rand_array
                            = _gen_rand_no( $seq_no, ( $#fp_clus_seq + 1 ) );
                        my @uniq = _remove_dupes( ( $#fp_clus_seq + 1 ),
                            @rand_array );
                        @temp = map { $fp_clus_seq[$_] } @uniq;
                        push( @fp_final, @temp );
                    }
                    my $out_fp_samp = Bio::SeqIO->new(
                        '-file'   => ">$path_train/neg.samp.seq",
                        '-format' => 'Fasta'
                    );
                    $out_fp_samp->write_seq(@fp_final);
                    $fpseqfile = "$path_train/neg.samp.seq";
                    print scalar @fp_final,
                ...
                system(
                    "muscle -in  $fpseqfile -maxiters 2 -maxmb 4200 -quiet -out $path_train/tot_fp.aln"
                );

                # next ..build an HMM from the alignment
                system(
                    "hmmbuild --informat afa $path_train/tot_fp.hmm $path_train/tot_fp.aln"
                );

           # ALign the 'False Positive' alignment with the 'True Positive' one
                my $aligned
                    = $clusterObj->mAlign( "muscle", \@params, $clus_no,
                    "$path_train/$outfile.aln", "$path_train/tot_fp.aln",
                    "p" );
...
# create a Bio::DB object from the combined alignment file to enable separation of the 'profile aligned' 'True Positive' & fp sequences
                ...
                my $tp_out = Bio::SeqIO->new(
                    '-file'   => ">$path_train/$outfile.tp.prof.aln",
                    '-format' => 'Fasta'
                );
                foreach my $id (@own_ids) {
                    my $seqobj
                        = $db->get_Seq_by_id($id);    # get a PrimarySeq obj
                    if ($seqobj) {
                        $tp_out->write_seq($seqobj)
                            ; # Fetch the 'True Positive' ids out from the profile-alignment and write it out as FASTA
                    }
                }

                # SeqIO object for 'False Positive' alignment
                my $fp_out = Bio::SeqIO->new(
                    '-file'   => ">$path_train/$outfile.fp.prof.aln",
                    '-format' => 'Fasta'
                );
                foreach my $id (@fp_ids) {
                    my $seqobj
                    ...
                    if ($seqobj) {
                        $fp_out->write_seq($seqobj)
                            ; # Fetch the 'False Positive' ids out from the profile-alignment and write it out as FASTA
                    }
                }

            # Build the HMM from the profile-aligned 'True positive' alignment
                system(
                    "hmmbuild --informat afa $path_train/$outfile.tp.prof.hmm $path_train/$outfile.tp.prof.aln"
                );

           # Build the HMM from the profile-aligned 'False positive' alignment
                system(
                    "hmmbuild --informat afa $path_train/$outfile.fp.prof.hmm $path_train/$outfile.fp.prof.aln"
                );

              # Modify the 'True positive' profile emission probabilites -ModE
                my $hmmmodefile = $clusterObj->modE(
                    $clus_no, "-",
                    "$path_train/$outfile.tp.prof.hmm",
                    "$path_train/$outfile.fp.prof.hmm"
                );
