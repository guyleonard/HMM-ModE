...
sub new {
    ...
    if ( !$clus_type ) {
        die
            "Clustering method not specified; Set -clusType => 'MCL' or 'o'\n";
    }
    my $clus_file = $param{-clusFile}
        || die "Cluster file not passed;quitting...\n";
    ...
    if ( $clus_type eq 'MCL' ) {
        if ( -s $tab_file ) {
            ...
            $mclheader = $file[0];
            if ( $mclheader !~ /\(mclheader/ ) {
                die "$self->{'clusfile'} not a valid MCL cluster file\n";
            }
            ...
            die
                "Cluster type is MCL, but the .tab file is missing; Can't proceed; Try -clusType => 'o'\n";
        }
    }
        ...
        if ( -s $clus_file ) {
            ...
            while ( $line = <FH> ) {
                ...
                if ( $. == 1 && $line !~ /Cluster/ ) {
                    die
                        "$clus_file does not match the prescribed format for -clusType => 'o'\n ";
                }
            ...
            die
                "Cluster type is 'o', but the cluster file is missing; Can't proceed; \n";
        }
    }
...
sub mAlign {
    ...
    my ( $self, $method, $ref_params, $clus_no, $seq1, $seq2, $prof_flag )
        = @_;
    ...
    if ( defined $seq1 && defined $seq2 && defined $prof_flag ) {
        ...
        if ( $method =~ /muscle/ ) {
            system(
                "muscle -profile -in1 $seq1 -in2 $seq2 -quiet -out $path_train/$out.both.aln"
            );
...
sub generateSamples {
    ...
    if ( $tot_seq >= 10 && $tot_fp >= 10 ) {
        ...
        if ( $tot_fp < $tot_seq ) {
            ...
            if   ($n) { $size_test_tp = ceil( $tot_seq / $n ); }
            else      { $n            = 0; }
            if ($size_test_tp) {
                $exhaustive_samples_tp = int( $tot_seq / $size_test_tp );
            }
            ...
            if   ($n) { $size_test_fp = ceil( $tot_fp / $n ); }
            else      { $n            = 0; }
            if ($size_test_fp) {
                $exhaustive_samples_fp = int( $tot_fp / $size_test_fp );
            }
    ...
    if   ($size_test_tp) { $left_over_tp = $tot_seq % $size_test_tp; }
    else                 { $left_over_tp = 0; }
    if   ($size_test_fp) { $left_over_fp = $tot_fp % $size_test_fp; }
    else                 { $left_over_fp = 0; }
    $samples_tp += $exhaustive_samples_tp;
    $samples_fp += $exhaustive_samples_fp;
    print
        "$n sample sets\t$tot_seq total sequences\t$size_test_tp => tp sample size\t$size_test_fp => fp sample size\t$exhaustive_samples_tp tp ex-samples possible\t$exhaustive_samples_fp fp ex-sampels possible\t$left_over_tp tp sequence(s) will be left out\t$left_over_fp fp sequence(s) will be left out \n";
    ...
    for ( my $set = 1; $set <= $exhaustive_samples_tp; $set++ ) {
        ...
        my $out_test = Bio::SeqIO->new(
            '-file'   => ">$path_test/$outfile.$set.test.seq",
            '-format' => 'Fasta'
        );
        ...
        my $out_train = Bio::SeqIO->new(
            '-file'   => ">$path_train/$outfile.$set.train.seq",
            '-format' => 'Fasta'
        );
    ...
    for ( my $set = 1; $set <= $exhaustive_samples_fp; $set++ ) {
        ...
        $sample_fp{$set} = \@test;
        my $out_test = Bio::SeqIO->new(
            '-file'   => ">$path_test/$outfile.$set.fp.seq",
            '-format' => 'Fasta'
        );
        $out_test->write_seq(@test);
        ...
        my $out_train = Bio::SeqIO->new(
            '-file'   => ">$path_train/$outfile.$set.fptrain.seq",
            '-format' => 'Fasta'
        );
        $out_train->write_seq(@train);
    ...
    if ($left_over_tp) {
        my ( $ref_test, $ref_train, $set_fill )
            = $self->_filler_samples( $cluster_no, $left_over_tp,
            $size_test_tp, \@seq, $samples_tp, "test", 1 );
        $samples_tp = $set_fill;
        my @test  = @$ref_test;
        my @train = @$ref_train;
    ...
    if ($left_over_fp) {
        my ( $ref_test, $set_fill )
            = $self->_filler_samples( $cluster_no, $left_over_fp,
            $size_test_fp, \@fp_seq, $samples_fp, "fp" );
        $samples_fp = $set_fill;
        my @test = @$ref_test;
    ...
    if ( $samples_tp < $n ) {
        ...
        while ( $samples_tp != $n ) {
            $samples_tp++;
            my ( $ref_test, $ref_train )
                = $self->splitSample( $cluster_no, $size_test_tp );
            my @test  = @$ref_test;
            my @train = @$ref_train;
            ...
            my $out_test = Bio::SeqIO->new(
                '-file'   => ">$path_test/$outfile.$samples_tp.test.seq",
                '-format' => 'Fasta'
            );
            $out_test->write_seq(@test);

            my $out_train = Bio::SeqIO->new(
                '-file'   => ">$path_train/$outfile.$samples_tp.train.seq",
                '-format' => 'Fasta'
            );
            $out_train->write_seq(@train);
    ...
    if ( $samples_fp < $n ) {
        ...
        while ( $samples_fp != $n ) {
            ...
            my $out_fp = Bio::SeqIO->new(
                '-file'   => ">$path_test/$outfile.$samples_fp.fp.seq",
                '-format' => 'Fasta'
            );
...
sub _filler_samples {
    my ( $self, $cluster_no, $left_over_seq, $sample_size, $seqref,
        $current_set, $string, $flag )
        = @_;
    ...
    push( @test, @rand_seq );
    my $out_test = Bio::SeqIO->new(
        '-file'   => ">$path_test/$outfile.$	set.$string.seq",
        '-format' => 'Fasta'
    );
    $out_test->write_seq(@test);
...
sub loadSamples {
    ...
    for ( my $samples = 1; $samples <= $tot_samples; $samples++ ) {
        ...
        my $in_test = Bio::SeqIO->new(
            '-file'   => "$path_test/$outfile.$samples.test.seq",
            '-format' => 'Fasta'
        );
        ...
        my $in_train = Bio::SeqIO->new(
            '-file'   => "$path_train/$outfile.$samples.train.seq",
            '-format' => 'Fasta'
        );
...
sub loadPath {
    ...
    if ( !$path_test && !$path_train ) {
        print
            "Path to store test and train samples not provided; will attempt to create /tmp/test_samples and /tmp/train_samples\n";
...
sub path {
    ...
    if ( !$path_train ) {
        print
            "Path to store tp profiles; will attempt to create /tmp/train_samples\n";
        _check_directory("/tmp/train_samples");
        $self->{'path_train'} = "/tmp/train_samples";
    }
    else {
...
sub _check_directory {
    ...
    {    # the -d bit means check if there is a directory called ...
        print
            "$directory already exists, samples will be saved in $directory\n";
    }
    else {
        print "$directory does not exist.....creating $directory.....\n\n";
        mkdir( $directory, 0777 )
            || die
            "sorry system is unable to create output directory $directory";
    }
...
sub crossValidate {
    ...
    foreach my $s_no (@samples) {
        ...
        my @params = (
            OUT      => "$path_train/$outfile.$s_no.tp.aln",
            MAXITERS => 2,
            "QUIET"  => "QUIET",
            MAXMB    => 3800
        );
        my $ref_params = \@params;
        my $train_aln  = $self->mAlign( "muscle", $ref_params, $cluster_no,
            $ref_train_array );
        system(
            "hmmbuild --informat afa $path_train/$outfile.$s_no.hmm $path_train/$outfile.$s_no.tp.aln"
        );
        my $ref_fp_test_hash = $self->hmmsearchParse(
            "$path_test/$outfile.$s_no.fp.seq",
            "$path_train/$outfile.$s_no.hmm",
            "neg.hmms"
        );
        my %fp_train_hash    = %$ref_fp_test_hash;
        my @fp_train_score   = values %fp_train_hash;
        my $ref_tp_test_hash = $self->hmmsearchParse(
            "$path_test/$outfile.$s_no.test.seq",
            "$path_train/$outfile.$s_no.hmm",
            "pos.hmms", -9999
        );
        my %tp_test_hash = %$ref_tp_test_hash;
        my @tp_score     = values %tp_test_hash;
        my %fp_test_hash = %$ref_fp_test_hash;
        my @fp_score     = values %fp_test_hash;
        my $ref_mcc      = $self->writeHisto( \@tp_score, \@fp_score,
            "$path_train/$outfile.$s_no.hmmer.histo" );
        %cumu_mcc = %$ref_mcc;
        ...
        if ( defined $fp_train_score[0] ) {
            ...
            @fp = @$ref_fptrain_seq;
            my @params = (
                OUT      => "$path_train/$outfile.$s_no.fp.aln",
                MAXITERS => 2,
                "QUIET"  => "QUIET",
                MAXMB    => 3800
            );
            my $fp_aln
                = $self->mAlign( "muscle", \@params, $cluster_no, \@fp );
            my $aligned = $self->mAlign(
                "muscle", \@params, $cluster_no,
                "$path_train/$outfile.$s_no.tp.aln",
                "$path_train/$outfile.$s_no.fp.aln", "p"
            );
            ...
            my $tp_out = Bio::SeqIO->new(
                '-file'   => ">$path_train/$outfile.$s_no.tp.prof.aln",
                '-format' => 'Fasta'
            );
            ...
            my $fp_out = Bio::SeqIO->new(
                '-file'   => ">$path_train/$outfile.$s_no.fp.prof.aln",
                '-format' => 'Fasta'
            );
            ...
            system(
                "hmmbuild --informat afa $path_train/$outfile.$s_no.tp.prof.hmm $path_train/$outfile.$s_no.tp.prof.aln"
            );
            system(
                "hmmbuild --informat afa $path_train/$outfile.$s_no.fp.prof.hmm $path_train/$outfile.$s_no.fp.prof.aln"
            );
            my $hmmmodefile = $self->modE(
                $cluster_no, $s_no,
                "$path_train/$outfile.$s_no.tp.prof.hmm",
                "$path_train/$outfile.$s_no.fp.prof.hmm"
            );
            my $ref_tp_test_hash
                = $self->hmmsearchParse( "$path_test/$outfile.$s_no.test.seq",
                "$hmmmodefile", "mode.pos.hmms", -9999 );
            my %tp_test_hash = %$ref_tp_test_hash;
            my @tp_score     = values %tp_test_hash;
            my $ref_fp_test_hash
                = $self->hmmsearchParse( "$path_test/$outfile.$s_no.fp.seq",
                "$hmmmodefile", "mode.neg.hmms", -9999 );
            my %fp_test_hash = %$ref_fp_test_hash;
            my @fp_score     = values %fp_test_hash;
            my $ref_mode_mcc = $self->writeHisto( \@tp_score, \@fp_score,
                "$path_train/$outfile.$s_no.mode.histo" );
    ...
    my $hmmer_cut
        = $self->_thresholdScore( \%seen, $cluster_no, "hmmer", $samples );
    if ($mode_counter) {
        my $mode_cut
            = $self->_thresholdScore( \%seen_mode, $cluster_no, "mode",
            $mode_counter );
...
sub modE {
    ...
    for ( my $i = 0; $i <= $#maps_tp; $i++ ) {
        ...
        for ( my $j = 0; $j <= scalar @{ $fp_hmm->{'BASE'} } - 1; $j++ ) {
            $REPositiveByNegative
                += ${ ${ $tp_hmm->matchEmissionProbablity }{ $maps_tp[$i] } }
                { ${ $tp_hmm->{'BASE'} }[$j] } *
                log(
                ${ ${ $tp_hmm->matchEmissionProbablity }{ $maps_tp[$i] } }
                    { ${ $tp_hmm->{'BASE'} }[$j] }
                    / ${ ${ $fp_hmm->matchEmissionProbablity }{ $maps_fp[$i] }
                    }{ ${ $fp_hmm->{'BASE'} }[$j] } );
        }
        ...
        if ( $REPositiveByNegative > $REPositiveByNull ) {
            ...
            my $denomenator = 0;
            for ( my $k = 0; $k <= scalar @{ $fp_hmm->{'BASE'} } - 1; $k++ ) {
                $newNull{ ${ $fp_hmm->{'BASE'} }[$k] }
                    = (
                    ${ $fp_hmm->nuleProbablity }{ ${ $fp_hmm->{'BASE'} }[$k] }
                        > ${
                        ${ $fp_hmm->matchEmissionProbablity }{ $maps_fp[$i] }
                        }{ ${ $fp_hmm->{'BASE'} }[$k] }
                    )
                ...
                if (${ $fp_hmm->nuleProbablity }{ ${ $fp_hmm->{'BASE'} }[$k] }
                    > ${ ${ $fp_hmm->matchEmissionProbablity }{ $maps_fp[$i] }
                    }{ ${ $fp_hmm->{'BASE'} }[$k] } )
                {
                    $newP{ ${ $tp_hmm->{'BASE'} }[$k] }
                        = ${ ${ $tp_hmm->matchEmissionProbablity }
                            { $maps_tp[$i] } }{ ${ $tp_hmm->{'BASE'} }[$k] };
                }
                else {
                    $newP{ ${ $tp_hmm->{'BASE'} }[$k] } = (
                        (   ${  ${ $tp_hmm->matchEmissionProbablity }
            ...
            while ( my ( $k, $v ) = each %newP ) {
                ...
                ${ ${ $tp_hmm->matchEmission }{ $maps_tp[$i] } }{$k}
                    = -log( $newP{$k} );
            }
        }
    }
...
sub hmmsearchParse {
    ...
    system("hmmsearch --cpu=1 --max $hmm $seq > $outfile");
    my $search
        = new Bio::SearchIO( -format => 'hmmer3', -file => "$outfile" );
...
sub _catSeq {
    ...
    my $ref_own_seq
        = $sample_id && $string
        ? $self->{ $string . "_samples" }{$cluster_id}{$sample_id}
        : $self->{'cluster_seq'}{$cluster_id};
    ...
    if ($internal_flag) {
        my $path
            ...
            : $self->{'path_test'};
        $seqin = Bio::SeqIO->new(
            -file   => "/bin/cat $path/*.$sample_id.$string.seq |",
            -format => 'Fasta'
        );
    }
    else {
...

{
    ...
    foreach my $range (@range) {
        ...
        print FH $range, "\t", $bin{$range}{'SENS'}, "\t",
            $bin{$range}{'SPEC'}, "\t", $bin{$range}{'MCC'}, "\t", $TP, "\t",
            $TN, "\t", $FP, "\t", $FN, "\n";
    }
    return \%mcc;
}
