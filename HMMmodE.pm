...
#----------------------------------------------------------------------------
package HMMmodE;
use lib '/home/swati/BioPerl-1.6.901/';
use lib '/home/swati/BioPerl-Run-1.006900/blib/lib/';
use Bio::DB::Fasta;
...
my @params=@$ref_params;
	...
	my $path_train=$self->{'path_train'};
		...
		if ($method =~ /muscle/)
		{	
		system ("muscle -profile -in1 $seq1 -in2 $seq2 -quiet -out $path_train/$out.both.aln");
...
my $mode_counter=0;
	...
	my $train_aln=$self->mAlign("muscle",$ref_params,$cluster_no,$ref_train_array);
        system ("hmmbuild --informat afa $path_train/$outfile.$s_no.hmm $path_train/$outfile.$s_no.tp.aln");
	my $ref_fp_test_hash=$self->hmmsearchParse("$path_test/$outfile.$s_no.fp.seq","$path_train/$outfile.$s_no.hmm","neg.hmms");
	my %fp_train_hash=%$ref_fp_test_hash;
	my @fp_train_score=values %fp_train_hash;
	my $ref_tp_test_hash=$self->hmmsearchParse("$path_test/$outfile.$s_no.test.seq","$path_train/$outfile.$s_no.hmm", "pos.hmms",-9999);
	...
	#	@fp=@$ref_fp_seq;
		...
		my $fp_aln=$self->mAlign("muscle",\@params,$cluster_no,\@fp);
		my $aligned=$self->mAlign("muscle",\@params,$cluster_no,"$path_train/$outfile.$s_no.tp.aln","$path_train/$outfile.$s_no.fp.aln","p");
		...
		my $fp_out=Bio::SeqIO->new('-file' => ">$path_train/$outfile.$s_no.fp.prof.aln",'-format' => 'Fasta');
 		...
 		system ("hmmbuild --informat afa $path_train/$outfile.$s_no.tp.prof.hmm $path_train/$outfile.$s_no.tp.prof.aln");
 	 	system ("hmmbuild --informat afa $path_train/$outfile.$s_no.fp.prof.hmm $path_train/$outfile.$s_no.fp.prof.aln");
		my $hmmmodefile=$self->modE($cluster_no,$s_no,"$path_train/$outfile.$s_no.tp.prof.hmm","$path_train/$outfile.$s_no.fp.prof.hmm");
		my $ref_tp_test_hash=$self->hmmsearchParse("$path_test/$outfile.$s_no.test.seq","$hmmmodefile","mode.pos.hmms",-9999);
		my %tp_test_hash=%$ref_tp_test_hash;
		my @tp_score=values %tp_test_hash;
		my $ref_fp_test_hash=$self->hmmsearchParse("$path_test/$outfile.$s_no.fp.seq","$hmmmodefile","mode.neg.hmms",-9999);
		my %fp_test_hash=%$ref_fp_test_hash;
		my @fp_score=values %fp_test_hash;
